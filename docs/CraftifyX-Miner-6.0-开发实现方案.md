# CraftifyX Miner 6.0 国产模型版 -- 完整开发实现方案
...
| Fan Creator Ratio | 初版简化: 无法直接获取，默认 0.5; 后续有 followers 采样后计算 |
...
- SPS = sum(weight_i * score_i) for 10 dimensions
- 中心度: SQL 查询 `creator_graph` 中被多少 Seed 关注 -> Hub(>=5) / Connector(2-4) / Peripheral(0-1)
...
- **Fan Creator Ratio**: 默认 0.5 占位，待 Followers 采样数据积累后计算 -> v1.1
...
3. Fan Creator Ratio 基于 Followers 采样计算
