...
CREATE TABLE IF NOT EXISTS creator_features (
    ...
    avg_daily_posts_30d FLOAT,
    reply_engagement_rate FLOAT,
    account_age_days_log FLOAT,
    has_shop_link FLOAT,
    is_nsfw FLOAT,
    is_multi_platform FLOAT,
    market_tier_high FLOAT,
    market_tier_mid FLOAT,
    market_tier_low FLOAT,
    content_furry FLOAT,
    content_anime FLOAT,
    content_vtuber FLOAT,
    content_gaming FLOAT,
    content_webcomic FLOAT,
    content_bl FLOAT,
    content_gl FLOAT,
    content_nsfw FLOAT
);
...
-- 19. creators_detail (高价值创作者详情档案)
-- 仅收录 is_seed=true 或 bd_decision='interested' 的创作者，便于后续精细化运营与深度分析。
CREATE TABLE IF NOT EXISTS creators_detail (
    ...
    region TEXT,
    timezone TEXT,
    market_tier_high BOOLEAN,
    market_tier_mid BOOLEAN,
    market_tier_low BOOLEAN,
    -- 粉丝与互动
    followers INTEGER,
    following INTEGER,
    tweets_count INTEGER,
    ...
    social_links JSONB,
    -- DNA 内容分类（多选，最多 3 个）
    content_furry BOOLEAN,
    content_anime BOOLEAN,
    content_vtuber BOOLEAN,
    content_gaming BOOLEAN,
    content_webcomic BOOLEAN,
    content_bl BOOLEAN,
    content_gl BOOLEAN,
    content_nsfw BOOLEAN,
    content_classifications TEXT[],
    -- 原始数据
    raw_profile JSONB,
    raw_dna_analysis JSONB,
    -- 元数据
